...
Aplikacja webowa oparta o Spring Boot, służąca do obsługi biblioteki: zarządzania czytelnikami, książkami, autorami,
kategoriami oraz procesem wypożyczania (w tym kolejkami do książek). Dokumentacja poniżej opisuje stan faktyczny na
podstawie kodu w pakiecie `com.betoniarka.biblioteka`.
...
W projekcie zaimplementowano kompletny model obiektowy i bazodanowy dla kluczowych pojęć biblioteki. Wszystkie encje są
mapowane przy użyciu JPA.
...
Repozytorium: `AppUserRepository` (rozszerza `JpaRepository<AppUser, Long>`) zapewnia podstawowe operacje CRUD oraz
...
- **Kompletny model:**
    - obiektowy – encje `AppUser`, `Book`, `Category`, `Author`, `BorrowedBook`, `QueueEntry`, `Review` odzwierciedlają
      kluczowe pojęcia systemu biblioteki,
    - bazodanowy – wszystkie encje są zmapowane na tabele JPA z relacjami (w tym wiele–do–wielu dla książek i
      kategorii).
