...
public class AppUserReportService {
    ...
    public AppUserSummaryReportDto getSummary() {
        ...
        long totalAppUsersWithBorrows = appUsersList.stream()
                                                    .filter(user -> !user.getCurrentBorrows().isEmpty())
                                                    .count();

        long totalAppUsersWithoutBorrows = totalAppUsers - totalAppUsersWithBorrows;

        long totalAppUsersWithOverdue = appUsersList.stream()
                                                    .filter(user -> {
                                                        for (Borrow borrow : user.getCurrentBorrows()) {
                                                            if (borrow.getBorrowedAt().plus(borrow.getBorrowDuration()).isBefore(Instant.now()))
                                                                return true;
                                                        }
                                                        return false;
                                                    }).count();
        ...
        long totalOverdueDays = borrowRepository.findAll().stream()
                                                .filter(Borrow::isReturned)
                                                .mapToLong(borrow -> {
                                                    Instant returnedTime = borrow.getReturnedAt();
                                                    Instant expectedReturnTime = borrow.getBorrowedAt().plus(borrow.getBorrowDuration());
    ...
    public List<AppUserWithOverdueDto> getOverdue() {
        ...
        return borrowRepository.findAll().stream()
                                  .filter(borrow -> borrow.isReturned() &&
                                          Duration.between(Instant.now(), borrow.getBorrowedAt().plus(borrow.getBorrowDuration())).isPositive()
                                  )
    ...
    public List<MostActiveAppUserDto> getMostActive(int limit) {
        ...
        return userRepository.findAll().stream()
                                       .sorted(Comparator.comparingLong(
                                               (AppUser user) -> user.getBorrows().size()).reversed())
                                       .limit(limit)
                                       .map(user -> new MostActiveAppUserDto(
                                               user.getId(),
                                               user.getUsername(),
    ...
    public List<DeadAppUserAccountDto> getDead(long days) {
        ...
        return userRepository.findAll().stream()
                                       .filter(user -> user.getCurrentBorrows().isEmpty() &&
                                               Duration.between(user.getBorrows().getLast().getReturnedAt(), Instant.now()).toDays() >= days)
                                       .map(user -> new DeadAppUserAccountDto(
                                                user.getId(),
                                                user.getUsername(),
                                                Duration.between(user.getBorrows().getLast().getReturnedAt(), Instant.now()).toDays())
                                        ).toList();
