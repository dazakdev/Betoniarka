...
public class BookReportService {
    ...
    public List<MostReviewedBookDto> getMostReviewed(int limit) {
        ...
        return bookRepository.findAll().stream()
                                       .sorted(Comparator.comparingInt(
                                               (Book book) -> book.getReviews().size()).reversed()
                                       )
                                       .limit(limit)
                                       .map(book -> new MostReviewedBookDto(
                                               book.getId(),
                                               book.getTitle(),
    ...
    public List<MostPopularBookCategoryDto> getMostPopularCategories(int limit) {
        ...
        var categoryCountMap = borrowRepository.findAll().stream()
        ...
        return categoryCountMap.entrySet().stream()
                                          ...
                                          .limit(limit)
                                          .map(entry -> new MostPopularBookCategoryDto(
                                                  entry.getKey().getId(),
